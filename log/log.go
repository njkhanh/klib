...
func Start(level Level, path string) {
	doLogging(level, path, MaxBytes, BackupCount)
}

func StartEx(level Level, path string, maxBytes, backupCount int) {
	doLogging(level, path, maxBytes, backupCount)
}
...
func doLogging(logLevel Level, fileName string, maxBytes, backupCount int) {
...
func SError(format string, a ...interface{}) {
	Logger.Error.Output(2, fmt.Sprintf(format, a...))
}

func ErrorT(err error, a ...interface{}) {
	Logger.Error.Output(2, fmt.Sprint(err, a))
}
...
func Println(v ...interface{}) {
	Logger.Info.Output(2, fmt.Sprintln(v...))
