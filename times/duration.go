...
// Parse creates a new *Dura struct, returns error if input is invalid.
func Parse(dinput time.Duration) *Dura {
	input := dinput.String()
	return &Dura{dinput, input, false}
}

// ParseShort creates a new *Dura struct, short form, returns error if input is invalid.
func ParseShort(dinput time.Duration) *Dura {
	input := dinput.String()
	return &Dura{dinput, input, true}
}

// ParseString creates a new *Dura struct from a string.
// returns an error if input is invalid.
func ParseString(input string) (*Dura, error) {
	if input == "0" || input == "-0" {
		return nil, errors.New("missing unit in duration " + input)
	}
	duration, err := time.ParseDuration(input)
	if err != nil {
		return nil, err
	}
	return &Dura{duration, input, false}, nil
}

// ParseStringShort creates a new *Dura struct from a string, short form
// returns an error if input is invalid.
func ParseStringShort(input string) (*Dura, error) {
	if input == "0" || input == "-0" {
		return nil, errors.New("missing unit in duration " + input)
	}
	duration, err := time.ParseDuration(input)
	...
	return &Dura{duration, input, true}, nil
}

// String parses d *Dura into a human readable duration.
func (d *Dura) String() string {
	var duration string
